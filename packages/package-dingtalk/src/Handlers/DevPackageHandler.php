...
class DevPackageHandler extends \App\Support\PackageHandler
{
    ...
    public function makeMigrationHandle(Command $command)
    {
        ...
        $path = Path::relative(
            $this->app->basePath(),
            $this->app->make('path.schedule.migrations')
        );
    ...
    {
        $path = $this->app->make('path.schedule.seeds');
        ...
        $content = file_get_contents(
            $this->app->make('path.schedule.resources').'/stubs/seeder'
        );
    ...
    public function makeModelHandle(Command $command)
    {
        ...
        $content = file_get_contents(
            $this->app->make('path.schedule.resources').'/stubs/model'
        );
        $variable = [
            '{--name--}' => $modelName,
            '{--table-name--}' => $table,
